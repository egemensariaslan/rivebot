...
const play = {
	name: 'play',
	description: 'Şarkı oynat',
	async execute(message, arg, musicBot) {
		const {voiceChannel} = message.member;
		if (!voiceChannel) {
			return message.channel.send(
				new ErrorEmbed('Müzik oynatmak için bir sesli kanala giriniz.')
			);
		}
		...
		if (ytdl.validateURL(arg)) {
			...
			if (infos.results.length === 0) {
				return message.channel.send(
					new ErrorEmbed('Bu şarkı oynatılamıyor!')
				);
			}
		...
		if (!songInfo) {
			return message.channel.send(
				new ErrorEmbed('Bu şarkı kısıtlanmış veya bulunamadı!')
			);
		}
		...
		if (serverQueue === undefined) {
			...
			try {
				...
				message.channel.send(new ErrorEmbed('Bir hata meydana geldi!'));
			}
		} else {
